...
(with-ignore-coverage
 (defclass ssa-variable (operand)
     ((version :initarg :version :accessor ssa-variable-version :type (integer 0 *))
      (name :initarg :name :accessor ssa-variable-name :type string)
      (symbol :initarg :symbol :accessor ssa-variable-symbol))))

(defmethod print-operand ((v ssa-variable) s)
  "Print the SSA variable operand"
  (format s "SSA(~a,~a)" (ssa-variable-name v) (ssa-variable-version v)))

(defmethod make-ssa-variable (name version symbol)
  "Create an SSA variable"
  (make-instance 'ssa-variable :name name :version version :symbol symbol))
