...
(fiveam:test test-program
  (fiveam:is (set-program-types (parse-program (make-token-sequence (tokenize "fun main int { return 0; }"))))))

(fiveam:test test-function
  (fiveam:is (null (function-type-parameters (ast-node-type-info (assign-type (function-rule (make-token-sequence (tokenize "fun main int { return 0; }"))) (make-env))))))
  (fiveam:is (integer-type-p (function-type-return-type (ast-node-type-info (assign-type (function-rule (make-token-sequence (tokenize "fun main int { return 0; }"))) (make-env)))))))
