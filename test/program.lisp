...
(fiveam:test function-basic
  (let ((ast (fun-from "fun main int { return 0; }")))
    (when (fiveam:is (acc::function-node-p ast) "Function must be a function node")
...
(fiveam:test block-multi-statement
  (let ((ast (block-from "{ let x : int = 6; x = 7; return x; }")))
    (when (fiveam:is (acc::block-node-p ast) "Block must be a block node")
          (fiveam:is (= 3 (length (acc::block-node-stmtlist ast))) "Block must have three statements"))))